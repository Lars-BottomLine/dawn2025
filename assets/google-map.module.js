...
function calculateMapCenter(marker_data) {

    if (debug)console.log('calculateMapCenter: ', marker_data);

    // collect all positions
    marker_data.forEach(marker => {
        if (debug) console.log('calculateMapCenter: ', marker);
    ...
    positions.forEach(position => {
        if (position[0] < rect.x_min) {
            rect.x_min = position[0];
        }
        if (position[0] > rect.x_max) {
            rect.x_max = position[0];
        }
        if (position[1] < rect.y_min) {
            rect.y_min = position[1];
        }
        if (position[1] > rect.y_max) {
            rect.y_max = position[1];
        }
    })

    // now calc center
    const center_x = rect.x_min + (rect.x_max - rect.x_min) / 2;
    const center_y = rect.y_min + (rect.y_max - rect.y_min) / 2;
 ...
 * @param marker_data
 */
async function addMarker(map, marker_data) {
...
async function initMap() {
    ...
    const map_center = calculateMapCenter( window.map_data.markers );

    const map = new Map(document.getElementById('venuemap'), {
        center: map_center,
        zoom: 13,
